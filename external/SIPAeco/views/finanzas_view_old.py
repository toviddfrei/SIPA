...
class FinanzasTab(QWidget):
    ...
    def init_ui(self):
        """Construye la interfaz adaptada al tema oscuro de SIPA general."""
        layout_principal = QVBoxLayout(self)
        layout_principal.setContentsMargins(10, 10, 10, 10)
        layout_principal.setSpacing(10)
        
        # =====================================================================
        # 1. FORMULARIO DE ENTRADA (Heredando el tema dinámico global)
        # =====================================================================
        frame_form = QFrame()
        frame_form.setObjectName("FormFinanzasFrame")
        ...
        # Etiquetas informativas dinámicas integradas en la cabecera con contrastes limpios
        self.lbl_auto_proyecto = QLabel("<b>Proyecto:</b> -")
        self.lbl_auto_proyecto.setStyleSheet("color: #0969da; padding: 4px; border-radius: 4px;")
        self.lbl_auto_crono = QLabel("<b>Crono Tipo:</b> -")
        self.lbl_auto_crono.setStyleSheet("color: #0969da; padding: 4px; border-radius: 4px;")
        ...
        self.lbl_titulo_tabla.setStyleSheet("font-size: 13px;")
    ...
    def refresh_data(self, hitos_dict, catalogos):
        ...
        lista_hitos_ordenados = sorted(list(self.hitos_cache.keys()))
        for h_id in lista_hitos_ordenados:
            nombre_breve = self.hitos_cache[h_id].get("nombre_accion", "")[:30]
        ...
        texto_activo = self.combo_vincular_hito.currentText()
        if texto_activo:
            sincronizar_contexto_hito_labels(texto_activo, self.hitos_cache, self.lbl_auto_proyecto, self.lbl_auto_crono)
        ...
        # Aplicamos estilos dinámicos de alerta perimetral sin romper el fondo del frame
        self.card_desviacion.setStyleSheet("QFrame { border: 2px solid #2da44e; border-radius: 6px; }" if desv >= 0 else "QFrame { border: 2px solid #ef4444; border-radius: 6px; }")
        self.lbl_kpi_desviacion.setStyleSheet("font-size: 18px; font-weight: bold; color: #1a7f37;" if desv >= 0 else "font-size: 18px; font-weight: bold; color: #ef4444;")
        
        self.renderizar_pagina_actual()

    def renderizar_pagina_actual(self):
        """Pinta la matriz financiera de forma limpia sin saturar el hilo principal."""
        self.table_finanzas.setSortingEnabled(False)
        ...
        for local_row_idx, tx in enumerate(subconjunto_registros):
            ...
            if cuenta_tx == "SISTEMA":
                ...
                # 🛡️ BLINDAJE DE SEÑALES ANIDADO: Evita el re-disparo cíclico recursivo hacia la ventana madre
    ...
    def abrir_ventana_auditoria(self):
        ...
        for row, tx in enumerate(historico):
            ...
            # Bloqueamos para inicializar limpiamente la vista fría
            c_combo.blockSignals(True)
            c_combo.currentTextChanged.connect(al_cambiar_historico)
            c_combo.blockSignals(False)
            ...
            item_imp.setForeground(QColor("#1a7f37") if importe_clean >= 0 else QColor("#ef4444"))
