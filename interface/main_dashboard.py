...
# Sincronización absoluta entre Roadmap y archivos .md mediante
# el campo 'vinculo_archivo' de la base de datos.
...
class MainDashboard:
    def __init__(self, logs_inicio=None):
        ...
        # 1. Instanciar y 2. CONECTAR (capturando el éxito)
    ...
    # ----------------------------------------------------------------------
    # MOTOR UNIFICADO DE EDICIÓN (REFINADO PARA EXCELENCIA)
    # ----------------------------------------------------------------------
    def abrir_editor(self, event=None, source="sipadoc"):
        try:
            if source == "roadmap":
                ...
                # REGLA DE ORO: Obtenemos el nombre del archivo desde la columna oculta (índice 5)
                filename = val[5] 
                db_id = val[0]
                
                # Plan B: Si no hay vínculo, intentamos reconstruir por fecha para hilos antiguos
            ...
            # Si el archivo no existe, lo creamos con cabecera estándar
    ...
    def _init_tab_roadmap(self):
        ...
        # Definimos las columnas incluyendo el vínculo oculto
        ...
        self.tabla_road.heading("vinculo", text="Archivo") # Opcional: mostrar u ocultar
        
        self.tabla_road.column("id", width=40)
        self.tabla_road.column("vinculo", width=0, stretch=tk.NO) # Columna oculta por excelencia
    ...
    def cargar_roadmap_db(self):
        ...
        try:
            # Obtiene: (id, fecha, n1, n2, tarea, vinculo) de persistence.py
    ...
    def on_roadmap_select(self, event):
        ...
        try:
            ...
            self.txt_road_detail.insert(tk.END, detalle if detalle else "Documentación vinculada disponible mediante doble clic.")
        except: pass
